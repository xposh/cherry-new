...
const scoreCalculation_1 = require("../services/scoreCalculation");
...
async function getDisplayName(userId) {
  ...
  `;
    if (!user)
        return "Jemand";
    if (user.role === "talent") {
        const [profile] = await (0, db_1.default) `
...
router.get("/", authMiddleware_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (targetRole === "talent") {
            rawProfiles = await (0, db_1.default) `
        SELECT
          u.id,
          u.email,
      ...
      `;
        ...
        const cleanedProfiles = rawProfiles
            ...
            if (targetRole === "talent") {
                const rawImg = String(d.profileImage || d.imageUrl || d.main_image_url || "");
                const cleanImg = rawImg.replace(/\/Photographer\//g, "/photographer/");
                const fallbackName = String(d.name ||
                    d.fullName ||
                    d.full_name ||
                    row.email ||
                    "Registrierter User");
                ...
                const fallbackCompany = String(d.companyName || d.name || row.email || "Registriertes Unternehmen");
                if (search &&
...
router.get("/matches", authMiddleware_1.requireAuth, async (req, res) => {
    const userId = req.auth.userId;
    const role = req.auth.role;
    const partnerType = role === "talent" ? "company" : "talent";
    try {
        const rows = role === "talent"
        ...
        const matches = rows.map((row) => {
            const d = safelyParseJSON(row.data);
            const name = partnerType === "talent"
                ? String(d.name || row.email)
                : String(d.companyName || row.email);
            const image = partnerType === "talent"
...
router.post("/interact", authMiddleware_1.requireAuth, async (req, res) => {
    const fromId = req.auth.userId;
    ...
    `;
        await (0, scoreCalculation_1.trackDailyEngagement)(fromId).catch((err) => console.error("trackDailyEngagement (interact) fehlgeschlagen:", err));
        if (action !== "like") {
            return res.json({ status: "skipped" });
        }
        getDisplayName(fromId)
            .then((fromName) => (0, scoreCalculation_1.addActivityToFeed)(targetId, "like", `${fromName} liked your profile`, fromId, fromName))
            .catch((err) => console.error("addActivityToFeed (like) fehlgeschlagen:", err));
        const [mutual] = await (0, db_1.default) `
    ...
    `;
        ...
        const [newMatch] = await (0, db_1.default) `
    ...
    `;
        Promise.all([getDisplayName(fromId), getDisplayName(targetId)])
            .then(([fromName, targetName]) => Promise.all([
            (0, scoreCalculation_1.addActivityToFeed)(fromId, "match", `You matched with ${targetName}!`, targetId, targetName),
            (0, scoreCalculation_1.addActivityToFeed)(targetId, "match", `You matched with ${fromName}!`, fromId, fromName),
            (0, scoreCalculation_1.trackDailyEngagement)(targetId),
        ]))
            .catch((err) => console.error("Match-Activity-Feed fehlgeschlagen:", err));
...
router.get("/public/:id", async (req, res) => {
    ...
    try {
        const [talentRow] = await (0, db_1.default) `
    ...
    `;
        if (talentRow) {
            ...
            if (!cleanData.name)
                cleanData.name = talentRow.email;
            return res.json({ profile: cleanData, type: "talent" });
        }
        const [companyRow] = await (0, db_1.default) `
    ...
    `;
        if (companyRow) {
            ...
            if (!cleanData.companyName)
                cleanData.companyName = companyRow.email;
    ...
    `;
        if (talentRow) {
            ...
            if (!cleanData.name)
                cleanData.name = talentRow.email;
            if (viewerId !== id) {
                (0, scoreCalculation_1.trackProfileView)(id, viewerId).catch((err) => console.error("trackProfileView (talent) fehlgeschlagen:", err));
                ...
                (0, scoreCalculation_1.trackDailyEngagement)(viewerId).catch((err) => console.error("trackDailyEngagement (browsing) fehlgeschlagen:", err));
            }
            return res.json({ profile: cleanData, type: "talent" });
        }
        const [companyRow] = await (0, db_1.default) `
    ...
    `;
        if (companyRow) {
            ...
            if (!cleanData.companyName)
                cleanData.companyName = companyRow.email;
            if (viewerId !== id) {
                (0, scoreCalculation_1.trackProfileView)(id, viewerId).catch((err) => console.error("trackProfileView (company) fehlgeschlagen:", err));
                // Ich markiere hier zusätzlich meinen eigenen Tag als aktiv — bisher wurde
                // nur bei Like/Skip Engagement getrackt, wodurch reines Browsen ohne
                // Interaktion nie in der Heatmap auftauchte.
                (0, scoreCalculation_1.trackDailyEngagement)(viewerId).catch((err) => console.error("trackDailyEngagement (browsing) fehlgeschlagen:", err));
