...
function safelyParseProfileData(input) {
    if (!input)
        return {};
...
function hasOverlap(needles, haystackText) {
    if (!needles.length || !haystackText)
        return false;
...
router.get("/screen-time", authMiddleware_1.requireAuth, async (req, res) => {
    ...
    `;
        ...
        let color = "#B8C98A"; // khaki-gruen — bester Zustand
        if (totalMinutes > 120) {
...
router.get("/response-reliability", authMiddleware_1.requireAuth, async (req, res) => {
    try {
      ...
      SELECT
        ...
        COUNT(*) as total
      FROM matches m
      WHERE m.${(0, db_1.default)(matchField)} = ${userId}
...
router.get("/handpicked-opportunities", authMiddleware_1.requireAuth, async (req, res) => {
    try {
        ...
        const userRole = req.auth?.role;
        if (userRole === "talent") {
            const [viewerRaw] = await (0, db_1.default) `
        ...
        `;
            const ranked = candidates
                ...
                const companyName = String(companyData.companyName || companyData.name || row.email || "Company");
                const locationRaw = String(companyData.location || "");
                const { city, country } = splitLocation(locationRaw);
                const jobRole = String(row.title || companyData.jobTitle || companyData.industry || "Opportunity");
            ...
            const fallback = await (0, db_1.default) `
          SELECT
            company_name,
            company_city,
            company_country,
        ...
        `;
            const fallbackWithScore = fallback
                .map((row, idx) => {
                const descriptor = normalizeText([row.job_role, ...(Array.isArray(row.target_skills) ? row.target_skills : [])].join(" "));
                const roleFit = viewerFocusKeywords.length === 0 ||
        ...
        `;
            const extra = profileFallback
                ...
                const locationRaw = String(companyData.location || "");
                const { city, country } = splitLocation(locationRaw);
                const descriptor = normalizeText([companyData.industry, companyData.description, companyData.jobTitle].join(" "));
                const roleFit = viewerFocusKeywords.length === 0 ||
                    hasOverlap(viewerFocusKeywords, descriptor);
                if (!roleFit)
                    return null;
            ...
            for (const item of extra) {
                if (final.length >= 3)
                    break;
                const exists = final.some((f) => normalizeText(f.company_name) === normalizeText(item.company_name));
                if (!exists)
                    final.push(item);
            }
        ...
        `;
            ...
            const talents = await (0, db_1.default) `
          SELECT
            u.id,
            u.email,
        ...
        `;
            const ranked = talents
                ...
                const profileText = normalizeText([position, data.about, ...(Array.isArray(data.skills) ? data.skills : [])].join(" "));
                ...
                if (data.video_url)
                    score += 7;
                else if (data.profileImage || data.imageUrl)
                    score += 4;
                score = Math.min(score, 100);
                return {
                    name,
